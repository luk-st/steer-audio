...
@torch.no_grad()
def ddim_inversion(ldm_model, w0, prompts, cfg_scale, num_inference_steps, skip, duration=None):
    # Setup extra inputs for stable-audio models
    if hasattr(ldm_model, 'setup_extra_inputs') and 'stable-audio' in ldm_model.model_id:
        ldm_model.setup_extra_inputs(w0, init_timestep=ldm_model.model.scheduler.timesteps[0], audio_end_in_s=duration)

    _, text_emb, uncond_emb = get_text_embeddings(prompts, [""], ldm_model)

    latent = w0.clone().detach()

    # Diffusers DPMSolverMultistepInverseScheduler -> use step() directly
    if hasattr(ldm_model.model.scheduler, 'sigmas') and 'Inverse' in type(ldm_model.model.scheduler).__name__:
        for t in tqdm(ldm_model.model.scheduler.timesteps[:num_inference_steps-skip], disable=DISABLE_TQDM):
            noise_pred = get_noise_pred(ldm_model, latent, t, text_emb, uncond_emb, cfg_scale)
            latent = ldm_model.model.scheduler.step(noise_pred, t, latent).prev_sample
    else:
        # Original DDIM inversion for other schedulers
        for i in tqdm(range(num_inference_steps), disable=DISABLE_TQDM):
...
def text2image_ldm_stable(ldm_model, prompt: List[str], num_inference_steps: int = 50,
                          guidance_scale: float = 7.5, xt: Optional[torch.FloatTensor] = None, skip: int = 0, duration=None):
    # Setup extra inputs for stable-audio models
    if hasattr(ldm_model, 'setup_extra_inputs') and 'stable-audio' in ldm_model.model_id:
        ldm_model.setup_extra_inputs(xt, init_timestep=ldm_model.model.scheduler.timesteps[skip], audio_end_in_s=duration)

    _, text_emb, uncond_emb = get_text_embeddings(prompt, [""], ldm_model)

    for t in tqdm(ldm_model.model.scheduler.timesteps[skip:], disable=DISABLE_TQDM):
        noise_pred_uncond, _, _ = ldm_model.unet_forward(
                xt,
                timestep=t,
                encoder_hidden_states=uncond_emb.embedding_hidden_states,
        ...
        kwargs = {}
        if "DDIM" in type(ldm_model.model.scheduler).__name__.upper():
            kwargs['eta'] = 0
        xt = ldm_model.model.scheduler.step(noise_pred, t, xt, **kwargs).prev_sample
