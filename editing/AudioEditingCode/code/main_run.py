...
HF_TOKEN = os.getenv("HF_TOKEN")  # Needed for stable audio open. You can leave None when not using it
...
if __name__ == "__main__":
    ...
    is_stable_ddim = (args.mode == "ddim" and 'stable-audio' in model_id)

    ldm_stable = load_model(model_id, device, args.num_diffusion_steps, token=HF_TOKEN, ddim_inversion=is_stable_ddim)

    # DDIM+StableAudio returns artifacts, we use DPMSolverMultistepScheduler for denoising and DPMSolverMultistepInverseScheduler for inversion
    if is_stable_ddim:
        ldm_stable_inverse = load_model(model_id, device, args.num_diffusion_steps, token=HF_TOKEN, use_inverse_scheduler=True)
    else:
    ...
    with inference_mode():
        ...
        if args.mode == "ddim":
            ...
            wT = ddim_inversion(ldm_stable_inverse, w0, args.source_prompt, cfg_scale_src[0],
                                num_inference_steps=args.num_diffusion_steps, skip=skip[0], duration=duration)
        else:
