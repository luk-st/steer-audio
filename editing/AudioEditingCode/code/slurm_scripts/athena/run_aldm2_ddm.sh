        ...
        for cfg_tar in "${CFG_TARS[@]}"; do
            for hooks in "${HOOKS[@]}"; do
                for part in "${PARTS[@]}"; do
                    RUN_NAME="ldm_${METHOD}${hooks}_cfgs${cfg_src}_cfgt${cfg_tar}_ts${tstart}_step${NUM_DIFFUSION_STEPS}"
...
python edit_audioldm_medleydb.py --mode ${METHOD} --part_id ${part} --n_parts ${N_PARTS} --tstart ${tstart} --cfg_src ${cfg_src} --cfg_tar ${cfg_tar} --num_diffusion_steps ${NUM_DIFFUSION_STEPS} --seed ${SEED} --run_name ${RUN_NAME} ${hooks}
EOT
                done
            done
        done
