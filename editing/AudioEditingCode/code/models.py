...
class PipelineWrapper(torch.nn.Module):
    ...
    def get_noise_shape(self, x0: torch.Tensor, num_steps: int) -> Tuple[int, ...]:
        variance_noise_shape = (num_steps, self.model.unet.config.in_channels, x0.shape[-2], x0.shape[-1])
    ...
    def reverse_step_with_custom_noise(
        self,
        ...
        **kwargs,
        ...
        prev_timestep = (
            timestep - self.model.scheduler.config.num_train_timesteps // self.model.scheduler.num_inference_steps
        )
    ...
    def unet_forward(
        ...
        if self.model.unet.encoder_hid_proj is not None and self.model.unet.config.encoder_hid_dim_type == "text_proj":
            ...
            self.model.unet.encoder_hid_proj is not None
            and self.model.unet.config.encoder_hid_dim_type == "text_image_proj"
        ):
            # Kadinsky 2.1 - style
            if "image_embeds" not in added_cond_kwargs:
                raise ValueError(
        ...
        for i, upsample_block in enumerate(self.model.unet.up_blocks):
            ...
            if zero_out_resconns is not None:
                if (
                    (type(zero_out_resconns) is int and i >= (zero_out_resconns - 1))
                    or type(zero_out_resconns) is list
                    and i in zero_out_resconns
                ):
...
class TangoWrapper(PipelineWrapper):
    ...
    def load_scheduler(self, ddim_inversion: bool = False) -> None:
    ...
    def encode_text(self, prompts: List[str], **kwargs) -> Tuple[None, Optional[torch.Tensor], None]:
        ...
        with torch.no_grad():
            text_encoding = self.model.text_encoder(
                text_input.input_ids.to(self.device), attention_mask=attention_mask.to(self.device)
            )[0]
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self.double_precision:
            self.model = AudioLDM2Pipeline.from_pretrained(
                self.model_id, torch_dtype=torch.float64, token=self.token
            ).to(self.device)
        else:
            try:
                self.model = AudioLDM2Pipeline.from_pretrained(
                    self.model_id, local_files_only=True, token=self.token
                ).to(self.device)
            except FileNotFoundError:
                self.model = AudioLDM2Pipeline.from_pretrained(
                    self.model_id, local_files_only=False, token=self.token
                ).to(self.device)
...
class CelebAHQWrapper(PipelineWrapper):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.vqvae = (
            VQModel.from_pretrained(self.model_id, subfolder="vqvae", token=self.token, torch_dtype=torch.float32)
            .to(self.device)
            .to(torch.float32)
        )
        ...
        self.model = (
            dummyModule(
                model=UNet2DModel.from_pretrained(
                    self.model_id, subfolder="unet", token=self.token, torch_dtype=torch.float32
                )
                .to(self.device)
                .to(torch.float32),
                scheduler=None,
...
class StableAudWrapper(PipelineWrapper):
    def __init__(self, *args, **kwargs) -> None:
        ...
        try:
            self.model = StableAudioPipeline.from_pretrained(
                self.model_id, token=self.token, local_files_only=True
            ).to(self.device)
        except FileNotFoundError:
            self.model = StableAudioPipeline.from_pretrained(
                self.model_id, token=self.token, local_files_only=False
            ).to(self.device)
    ...
    def setup_extra_inputs(
        self,
        x: torch.Tensor,
        init_timestep: torch.Tensor,
        extra_info: Optional[Any] = None,
        audio_start_in_s: float = 0,
        audio_end_in_s: Optional[float] = None,
    ) -> None:
        max_audio_length_in_s = (
            self.model.transformer.config.sample_size * self.model.vae.hop_length / self.model.vae.config.sampling_rate
        )
        ...
        self.audio_duration_embeds = torch.cat(
            [self.seconds_start_hidden_states, self.seconds_end_hidden_states], dim=2
        )
        ...
        # Initialize step index for DPMSolver schedulers
        if hasattr(self.model.scheduler, "_init_step_index"):
            ...
            if hasattr(self.model.scheduler, "model_outputs") and hasattr(self.model.scheduler, "lower_order_nums"):
                if extra_info is not None and len(extra_info) > 0:
                    # We have history from full inversion - use it for immediate 2nd order
                    ...
                    self.model.scheduler.model_outputs = [None, extra_info[idx] if idx < len(extra_info) else None]
                    self.model.scheduler.lower_order_nums = min(
                        self.model.scheduler.step_index, self.model.scheduler.config.solver_order
                    )
                else:
                    # No history (SDEdit/DDIM) - Natural Progression: Start 1st order, progress to 2nd order
                    # Initialize clean state for 2nd order scheduler
                    self.model.scheduler.model_outputs = [None] * self.model.scheduler.config.solver_order
                    self.model.scheduler.lower_order_nums = 0  # Start with 1st order
    ...
    def get_zs_from_xts(
        ...
        **kwargs,
        ...
        lower_order_second = (
            (curr_step_index == len(timesteps) - 2)
            and self.model.scheduler.config.lower_order_final
            and len(timesteps) < 15
        )
        ...
        if (curr_step_index == len(timesteps) - 1) and self.model.scheduler.config.final_sigmas_type == "zero":
            ...
            z = (
                xtm1
                - (sigma_t / sigma_s0 * torch.exp(-h)) * xt
    ...
    def reverse_step_with_custom_noise(
        ...
        **kwargs,
        ...
        lower_order_second = (
            (self.model.scheduler.step_index == len(self.model.scheduler.timesteps) - 2)
            and self.model.scheduler.config.lower_order_final
            and len(self.model.scheduler.timesteps) < 15
        )
        ...
        if variance_noise is None:
            if self.model.scheduler.noise_sampler is None:
                self.model.scheduler.noise_sampler = BrownianTreeNoiseSampler(
                    model_output,
                    sigma_min=self.model.scheduler.config.sigma_min,
                    sigma_max=self.model.scheduler.config.sigma_max,
                    seed=None,
                )
            variance_noise = self.model.scheduler.noise_sampler(
                self.model.scheduler.sigmas[self.model.scheduler.step_index],
                self.model.scheduler.sigmas[self.model.scheduler.step_index + 1],
            ).to(model_output.device)

        if (
            first_order
            or self.model.scheduler.config.solver_order == 1
            or self.model.scheduler.lower_order_nums < 1
            ...
            self.model.scheduler.config.solver_order == 2
            or self.model.scheduler.lower_order_nums < 2
    ...
    def unet_forward(
        ...
        **kwargs,
        ...
        embeds = torch.cat(
            [encoder_hidden_states, self.seconds_start_hidden_states, self.seconds_end_hidden_states], dim=1
        )
...
def load_model(
    ...
    if "tango" in model_id:
        ...
        assert edit_method is None or edit_method.lower() in [
            "ddpm",
            "sdedit",
            "ddim",
        ...
        ldm_stable = StableDiffWrapper(
            model_id=model_id, device=device, double_precision=double_precision, token=token
        )
    ...
    disable_progress_bar()
    ldm_stable.load_scheduler(ddim_inversion=(edit_method.lower() == "ddim"))
    if "stable-audio" in model_id:
        if edit_method.lower() in ["sdedit"]:
            SOLVER_ORDER = 1
            config = {
                **ldm_stable.model.scheduler.config,
                "solver_order": SOLVER_ORDER,
            }
            ldm_stable.model.scheduler = CosineDPMSolverMultistepScheduler.from_config(config)
...
def load_model_inversion(
    ...
    assert "stable-audio" in model_id and edit_method.lower() in [
        "ddim"
    ], "method only available for stable audio with ddim"
    disable_progress_bar()
    ldm_stable = StableAudWrapper(model_id=model_id, device=device, double_precision=double_precision, token=token)
    from diffusers import DPMSolverMultistepInverseScheduler

    ldm_stable.model.scheduler = DPMSolverMultistepInverseScheduler.from_config(ldm_stable.model.scheduler.config)
    ldm_stable.model.scheduler.set_timesteps(num_diffusion_steps, device=device)
    enable_progress_bar()
    torch.cuda.empty_cache()
    return ldm_stable
