...
def get_height_of_spectrogram(length: int, ldm_stable: PipelineWrapper) -> int:
    vocoder_upsample_factor = (
        np.prod(ldm_stable.model.vocoder.config.upsample_rates) / ldm_stable.model.vocoder.config.sampling_rate
    )

    if length is None:
        length = ldm_stable.model.unet.config.sample_size * ldm_stable.model.vae_scale_factor * vocoder_upsample_factor
...
def plot_corrs(
    args,
    corrs: List[np.array],
    in_corrs: List[List[torch.Tensor]],
    in_norms: List[List[torch.Tensor]],
    save_path: str,
    image_name_png: str,
    ...
    for ev_num in range(n_ev):
        ...
        logging_dict[f"convergence_{ev_num + 1}"] = wandb.plot.line_series(
            xs=np.arange(args.iters - 1),
            ys=ev_in_corrs,
            keys=np.arange(args.drift_start, args.drift_start - len(in_corrs), -1),
            title=f"Subspace iterations correlations PC#{ev_num + 1}",
            xname="iter",
        )
...
def get_text_embeddings(
    target_prompt: List[str], target_neg_prompt: List[str], ldm_stable: PipelineWrapper
    ...
    uncond_embedding_hidden_states, uncond_embedding_class_lables, uncond_boolean_prompt_mask = ldm_stable.encode_text(
        target_neg_prompt
    )

    text_emb = PromptEmbeddings(
        embedding_hidden_states=text_embeddings_hidden_states,
    ...
    uncond_emb = PromptEmbeddings(
        embedding_hidden_states=uncond_embedding_hidden_states,
