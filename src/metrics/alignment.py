# based on https://github.com/haoheliu/audioldm_eval/blob/main/audioldm_eval/eval.py
...
from audioldm_eval.audio.tools import load_json, load_pickle, save_pickle, write_json
from audioldm_eval.datasets.load_mel import MelPairedDataset, WaveDataset, load_npy_data
...
class MusicAlignmentEval:
    def __init__(self, sampling_rate, device, backbone="mert") -> None:
        ...
        if self.backbone == "mert":
            ...
            if self.sampling_rate == 16000:
                ...
                raise ValueError("We only support the evaluation on 16kHz and 32kHz sampling rate for CNN14.")
        else:
    ...
    def get_filename_intersection_ratio(self, dir1, dir2, threshold=0.99, limit_num=None):
        ...
        if len(intersect_keys) / len(keyset1) > threshold and len(intersect_keys) / len(keyset2) > threshold:
            ...
            print("There are no same number of files in both dirs. Processing two folder with same_name=False")
            return False
    ...
    def calculate_metrics(
        ...
        if calculate_kl is True:
            metric_kl, kl_ref, paths_1 = calculate_kl(featuresdict_1, featuresdict_2, "logits", same_name)

            out.update(metric_kl)

        if calculate_isc is True:
        ...
        result = {
            "frechet_distance": f"{out.get('frechet_distance', float('nan')):.3f}",
            "frechet_audio_distance": f"{out.get('frechet_audio_distance', float('nan')):.3f}",
            "kullback_leibler_divergence_sigmoid": f"{out.get('kullback_leibler_divergence_sigmoid', float('nan')):.3f}",
            "kullback_leibler_divergence_softmax": f"{out.get('kullback_leibler_divergence_softmax', float('nan')):.3f}",
            "lsd": f"{out.get('lsd', float('nan')):.3f}",
            "psnr": f"{out.get('psnr', float('nan')):.3f}",
        ...
        json_path = os.path.join(
            ...
            self.get_current_time() + "_" + os.path.basename(generate_files_path) + ".json",
        )
    ...
    def get_featuresdict(self, dataloader):
        ...
        for waveform, filename in tqdm(dataloader):
            try:
                ...
                with torch.no_grad():
                    if self.backbone == "mert":
                        ...
                        mert_input = self.processor(
                            waveform, sampling_rate=self.target_sample_rate, return_tensors="pt"
                        ).to(self.device)
                ...
                print("Classifier Inference error!")
                continue
...
if __name__ == "__main__":
    ...
    evaluator = MusicAlignmentEval(args.sampling_rate, device)
