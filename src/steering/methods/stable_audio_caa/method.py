"""Stable Audio Open CAA steering as a unified :class:`Controller`.

Wraps :class:`VectorStoreStableAudio` (forward-hook collector/applier on
``attn2`` outputs in the Stable Audio DiT) so CAA composes with
:class:`SteerableStableAudioModel` through the same surface as every ACE-Step
method.

Like AudioLDM2, Stable Audio does CFG by batching the conditional/unconditional
passes together (``[uncond..., cond...]``), so the controller slices the batch
dimension instead of tracking a CFG-pass counter.
"""
...
from src.models.stable_audio.stable_audio_steering.controller import (
    ...
    register_vector_control_stable_audio,
...
@register_method("stable_audio_caa")
class StableAudioCAASteeringController(Controller):
    """CAA steering on Stable Audio Open ``attn2`` cross-attention outputs."""
    ...
    def __init__(
        ...
        *,
        ...
        # Accept either a Stable Audio layer preset name or an explicit list of
        # module paths. resolve_stable_audio_layers() handles both.
        self.target_layers = target_layers
        self.device = device
        self._vector_store: VectorStoreStableAudio | None = None
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        """Load Stable Audio CAA steering vectors from disk or the Hub.

        Recognised layout matches ``StableAudioCaaScorer``::
    ...
    def mount(self, model: Any) -> None:
        """``model`` is the DiT — i.e. ``SteerableStableAudioModel.transformer``."""
        layer_names = resolve_stable_audio_layers(self.target_layers)
        self._vector_store = VectorStoreStableAudio(
        ...
        handles = register_vector_control_stable_audio(
            model,
            self._vector_store,
            layer_names,
            verbose=False,
    ...
    def generate(self, model: Any, **kwargs: Any) -> Any:
        """Run a single Stable Audio generation.

        Stable Audio CFG batches uncond+cond together; the controller needs to
        know whether CFG is active so it slices the batch correctly.
        """
        import torch

        if self._vector_store is None:
            raise RuntimeError(
                "StableAudioCAASteeringController.generate called before mount."
            )
        ...
        call_kwargs = {
            ...
            "num_inference_steps": int(
                kwargs.get("num_inference_steps", kwargs.get("infer_step", 100))
            ),
