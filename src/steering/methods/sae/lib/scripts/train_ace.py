...
@dataclass
class ACESaeConfig(SaeConfig):
    expansion_factor: int = 4
    batch_topk: bool = True
    k: int = 24

@dataclass
class ACETrainConfig(TrainConfig):
    sae: ACESaeConfig
    num_workers: int = 48
    lr: float = 3e-5
    lr_scheduler: str = "linear"
    auxk_alpha: float = 0.03125
    wandb_log_frequency: int = 250
    wandb_project: str = "music_sae"
    lr_warmup_steps: int = 1000

@dataclass
class ACERunConfig(ACETrainConfig):
    mixed_precision: str = "no"
    max_examples: int | None = None
    seed: int = 42
    device: str = "cuda"
    num_epochs: int = 10
    only_cond_preds: bool = True
    sae_mode: str = "sequence"  # "sequence" or "frequency"

    def __post_init__(self):
        super().__post_init__()
        assert self.run_name is not None
        self.run_name = self.sae_mode + "_" + self.run_name
...
def load_datasets_from_dirs(base_dirs, hookpoint, dtype=torch.float32):
    ...
    for base_dir in base_dirs:
        ...
        dataset.set_format(
            type="torch",
            columns=["activations", "timestep", "cfg_pass"],
            dtype=dtype,
        )
...
def run():
    ...
    if not ddp or rank == 0:
        for hookpoint in args.hookpoints:
            ...
            if args.sae_mode == "frequency":
                dataset = dataset.map(lambda x: {"activations": x["activations"].transpose(-2,-1)}, desc="Transposing activations for frequency mode SAE")
            elif args.sae_mode != "sequence":
                raise ValueError(f"Invalid SAE mode: {args.sae_mode}. Possible modes are 'sequence' and 'frequency'.")
            if args.only_cond_preds:
                dataset = dataset.filter(lambda x: x["cfg_pass"] == 0)
