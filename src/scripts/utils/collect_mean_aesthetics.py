...
def main(file_in: str, file_out: str, tf_name: str):
    ...
    file_in = Path(file_in) # type: ignore
    file_out = Path(file_out) # type: ignore

    df = pd.read_json(file_in, lines=True)
    results = {"name": tf_name}
    for metric in METRICS_AESTHETICS:
        results[f"{metric}_mean"] = df[metric].mean() # type: ignore
        results[f"{metric}_std"] = df[metric].std() # type: ignore
    if Path(file_out).exists(): # type: ignore
        df_results = pd.read_csv(file_out)
        df_results = pd.concat([df_results, pd.DataFrame(results, index=[0])], ignore_index=True) # type: ignore
    else:
        df_results = pd.DataFrame(results, index=[0]) # type: ignore
