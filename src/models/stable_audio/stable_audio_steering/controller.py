"""
CAA steering controller for Stable Audio Open via torch forward hooks:
...
_N_BLOCKS = len(STABLE_AUDIO_CROSS_ATTENTION_LAYERS)


def resolve_stable_audio_layers(layers: str) -> list[str]:
    """Maps short layer preset names to full layer names.

    Presets:
      - ``all``            : all 24 blocks
      - ``tf<i>``          : single block i, e.g. ``tf6``
      - ``tf<i>tf<j>``     : several blocks, e.g. ``tf11tf12tf13``
      - ``not_<tfspec>``   : all blocks EXCEPT those, e.g. ``not_tf11tf12tf13``
      - ``first12``        : blocks 0-11
      - ``last12``         : blocks 12-23
    """
    if layers == "all":
        return list(STABLE_AUDIO_CROSS_ATTENTION_LAYERS)
    if layers == "first12":
        return list(STABLE_AUDIO_CROSS_ATTENTION_LAYERS[:12])
    if layers == "last12":
        return list(STABLE_AUDIO_CROSS_ATTENTION_LAYERS[12:])

    def block_layer(idx: int) -> str:
        if not (0 <= idx < _N_BLOCKS):
            raise ValueError(f"Block index {idx} out of range [0, {_N_BLOCKS}).")
        return f".transformer.transformer_blocks.{idx}.attn2"

    # 'not_tf11tf12tf13' -> all blocks except {11, 12, 13}.
    if layers.startswith("not_"):
        excluded = set(resolve_stable_audio_layers(layers[len("not_") :]))
        return [l for l in STABLE_AUDIO_CROSS_ATTENTION_LAYERS if l not in excluded]

    # 'tf6' or 'tf11tf12tf13' -> parse each 'tf<i>' token.
    if layers.startswith("tf"):
        idxs = [int(tok) for tok in layers.split("tf") if tok != ""]
        return [block_layer(i) for i in idxs]

    raise ValueError(
        f"Unknown layers preset: {layers!r}. Options: 'all', 'first12', 'last12', "
        "'tf<i>' (e.g. 'tf6'), 'tf<i>tf<j>' (e.g. 'tf11tf12tf13'), "
        "'not_<tfspec>' (e.g. 'not_tf11tf12tf13')."
    )


class VectorStoreStableAudio:
    """
    Collects and/or applies steering vectors at Stable Audio cross-attention outputs.
    ...
    def __init__(
        ...
        # Set by SteeredStableAudioPipeline before each generate.
        self.do_cfg = True
...
def register_vector_control_stable_audio(
    transformer: torch.nn.Module,
    controller: VectorStoreStableAudio,
    ...
    ``layer_names`` entries follow the convention in
    ``STABLE_AUDIO_CROSS_ATTENTION_LAYERS`` (leading dot, including the
    ``transformer.`` prefix); we strip that so names resolve against
    ``transformer.named_modules()``.
    """
    name_to_module = dict(transformer.named_modules())
    hooks: list[torch.utils.hooks.RemovableHandle] = []
    registered: list[str] = []

    for full_name in layer_names:
        mod_name = full_name.lstrip(".")
        if mod_name.startswith("transformer."):
            mod_name = mod_name[len("transformer.") :]
        if mod_name not in name_to_module:
            if verbose:
                print(f"[register] WARNING: {full_name} not found in transformer; skipping")
            continue
...
def _make_hook(controller: VectorStoreStableAudio, layer_name: str):
