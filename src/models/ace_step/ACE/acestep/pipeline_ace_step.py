...
class ACEStepPipeline:
    ...
    def get_text_embeddings(self, texts, text_max_length=256):
        inputs = self.text_tokenizer(
            texts,
            return_tensors="pt",
            padding="max_length" if self.pad_to_max_len is not None else True,
            truncation=True,
            max_length=self.pad_to_max_len
    ...
    def calc_v(
        ...
        hook_target_encoder_hidden_states=None,
        hook_target_encoder_hidden_mask=None,
        hook_target_rotary_freqs_cis=None,
        layers_to_hook: list[str] | None = None,
    ):
        will_hook = layers_to_hook is not None
        noise_pred_src = None
        if return_src_pred:
        ...
        if will_hook:
            ...
            def create_cross_attn_hook(
                target_encoder_hidden_states,
                target_encoder_attention_mask,
                target_rotary_freqs_cis,
            ):
                def cross_attn_hook(module, args, kwargs):
                    ...
                    if "encoder_attention_mask" in kwargs:
                        kwargs["encoder_attention_mask"] = target_encoder_attention_mask
                    if "rotary_freqs_cis_cross" in kwargs:
                        kwargs["rotary_freqs_cis_cross"] = target_rotary_freqs_cis
                    return args, kwargs

                return cross_attn_hook

            layers_path_parts = [
                layer_to_hook.replace(".ace_step_transformer.", "").split(".")
                for layer_to_hook in layers_to_hook
            ]
            hook_handles = []
            for layer_path_parts in layers_path_parts:
                target_transformer_block = self.ace_step_transformer
                for part in layer_path_parts:
                    if part.isdigit():
                        target_transformer_block = target_transformer_block[int(part)]
                    else:
                        target_transformer_block = getattr(
                            target_transformer_block, part
                        )

                if hasattr(target_transformer_block, "cross_attn"):
                    hook_function = create_cross_attn_hook(
                        hook_target_encoder_hidden_states,
                        hook_target_encoder_hidden_mask,
                        hook_target_rotary_freqs_cis,
                    )
                    hook_handle = (
                        target_transformer_block.cross_attn.register_forward_pre_hook(
    ...
    def calc_v_ode(
        ...
        latent_model_input = torch.cat([zt, zt]) if do_classifier_free_guidance else zt
        timestep = t.expand(latent_model_input.shape[0])
        noise_pred = self.ace_step_transformer(
    ...
    def ode_inversion_invert(
        ...
        infer_steps=60,
        tstart=60,
        guidance_scale_inversion=1.0,
        cfg_type="apg",
    ):
        do_classifier_free_guidance = True
        if guidance_scale_inversion == 0.0 or guidance_scale_inversion == 1.0:
            do_classifier_free_guidance = False
        ...
        xt = src_latents.clone()
        all_inverted_latents = [xt]
        ...
        zero_tensor = torch.tensor([0.0], dtype=self.dtype, device=self.device)
        reversed_timesteps = torch.cat((zero_tensor, reversed_timesteps))

        steps_to_do = reversed_timesteps[: -(1 + T_steps - tstart)]
        next_steps = reversed_timesteps[1 : len(steps_to_do) + 1]

        for i, t in tqdm(enumerate(steps_to_do), total=len(steps_to_do), disable=True):
            t_i = t / 1000
            if i < len(next_steps):
                t_next_i = (next_steps[i]) / 1000
            else:
                raise ValueError(f"{i=} {steps_to_do=} {next_steps=}")

            # ode inversion step
            v_ode = self.calc_v_ode(
                zt=xt,
                t=t,
                encoder_text_hidden_states=encoder_text_hidden_states,
                text_attention_mask=text_attention_mask,
                ...
                lyric_mask=lyric_mask,
                guidance_scale=guidance_scale_inversion,
                cfg_type=cfg_type,
                attention_mask=attention_mask,
                momentum_buffer=momentum_buffer,
            )
            ...
            xt = xt + (t_next_i - t_i) * v_ode
            xt = xt.to(self.dtype)

            all_inverted_latents.append(xt)

        return xt, all_inverted_latents

    @torch.no_grad()
    def ode_inversion_sampling(
        self,
        source_encoder_text_hidden_states,
        source_text_attention_mask,
        source_speaker_embds,
        source_lyric_token_ids,
        source_lyric_mask,
        target_encoder_text_hidden_states,
        target_text_attention_mask,
        target_speaker_embds,
        target_lyric_token_ids,
        target_lyric_mask,
        src_latents,
        infer_steps=60,
        tstart=60,
        guidance_scale_sampling=1.0,
        cfg_type="apg",
        layers_to_hook: list[str] | None = None,
    ):
        will_hook = layers_to_hook is not None
        do_classifier_free_guidance = True
        if guidance_scale_sampling == 0.0 or guidance_scale_sampling == 1.0:
            do_classifier_free_guidance = False
        bsz = source_encoder_text_hidden_states.shape[0]
        ...
        if do_classifier_free_guidance:
            attention_mask = torch.cat([attention_mask] * 2, dim=0)
            if will_hook:
                source_encoder_text_hidden_states = torch.cat(
                    [
                        source_encoder_text_hidden_states,
                        torch.zeros_like(source_encoder_text_hidden_states),
                    ],
                    0,
                )
                source_text_attention_mask = torch.cat(
                    [
                        source_text_attention_mask,
                        torch.zeros_like(source_text_attention_mask),
                    ],
                    0,
                )
                source_speaker_embds = torch.cat(
                    [source_speaker_embds, torch.zeros_like(source_speaker_embds)], 0
                )
                source_lyric_token_ids = torch.cat(
                    [source_lyric_token_ids, torch.zeros_like(source_lyric_token_ids)],
                    0,
                )
                source_lyric_mask = torch.cat(
                    [source_lyric_mask, torch.zeros_like(source_lyric_mask)], 0
                )
            ...
            target_speaker_embds = torch.cat(
                [target_speaker_embds, torch.zeros_like(target_speaker_embds)], 0
            )
        ...
        if will_hook and len(layers_to_hook) > 0:
            hook_target_encoder_hidden_states, hook_target_encoder_hidden_mask = (
                self.ace_step_transformer.encode(
                    encoder_text_hidden_states=target_encoder_text_hidden_states,
                    text_attention_mask=target_text_attention_mask,
                    speaker_embeds=target_speaker_embds,
                    lyric_token_idx=target_lyric_token_ids,
                    lyric_mask=target_lyric_mask,
                )
            )
            hook_target_rotary_freqs_cis = self.ace_step_transformer.rotary_emb(
                hook_target_encoder_hidden_states,
                seq_len=hook_target_encoder_hidden_mask.shape[1],
            )

        momentum_buffer = MomentumBuffer()
        xt = src_latents.clone()
        all_sampled_latents = [xt]
        scheduler = FlowMatchEulerDiscreteScheduler(
            num_train_timesteps=1000,
            shift=3.0,
        )
        ...
        timesteps = torch.cat(
            (timesteps, torch.tensor([0.0], dtype=self.dtype, device=self.device))
        )
        steps_to_do = timesteps[(T_steps - tstart) : -1]
        next_steps = timesteps[(T_steps - tstart + 1) :]
        ...
        for i, t in tqdm(enumerate(steps_to_do), total=len(steps_to_do), disable=True):
            ...
            if will_hook:
                ...
                for layer_path_parts in layers_path_parts:
                    ...
                    for part in layer_path_parts:
                        if part.isdigit():
                            target_transformer_block = target_transformer_block[
                                int(part)
                            ]
                        else:
                            target_transformer_block = getattr(
                    ...
                    if hasattr(target_transformer_block, "cross_attn"):
                        ...
                        hook_handle = target_transformer_block.cross_attn.register_forward_pre_hook(
                            hook_function, with_kwargs=True
                        )
                        hook_handles.append(hook_handle)
                ...
                try:
                    v_ode = self.calc_v_ode(
                        zt=xt,
                        t=t,
                        encoder_text_hidden_states=source_encoder_text_hidden_states,
                        text_attention_mask=source_text_attention_mask,
                        speaker_embds=source_speaker_embds,
                        lyric_token_ids=source_lyric_token_ids,
                        lyric_mask=source_lyric_mask,
                        guidance_scale=guidance_scale_sampling,
                        cfg_type=cfg_type,
                        attention_mask=attention_mask,
                        momentum_buffer=momentum_buffer,
                    )
                ...
                v_ode = self.calc_v_ode(
                    ...
                    encoder_text_hidden_states=target_encoder_text_hidden_states,
                    text_attention_mask=target_text_attention_mask,
                    speaker_embds=target_speaker_embds,
                    lyric_token_ids=target_lyric_token_ids,
                    lyric_mask=target_lyric_mask,
                    guidance_scale=guidance_scale_sampling,
                    cfg_type=cfg_type,
                    attention_mask=attention_mask,
            ...
            all_sampled_latents.append(xt)

        return xt, all_sampled_latents
    ...
    def flowedit_diffusion_process(
        ...
        for i, t in tqdm(enumerate(timesteps), total=T_steps, disable=True):
            if i < n_min:
                continue
            ...
            if i < n_max:
                ...
                for k in range(n_avg):
                    ...
                    Vt_src, Vt_tar = self.calc_v(
                        ...
                        cfg_type=cfg_type,
                        hook_target_encoder_hidden_states=hook_target_encoder_hidden_states,
                        hook_target_encoder_hidden_mask=hook_target_encoder_hidden_mask,
                        hook_target_rotary_freqs_cis=hook_target_rotary_freqs_cis,
                    )
                    V_delta_avg += (1 / n_avg) * (
                        Vt_tar - Vt_src
                    )  # - (hfg - 1) * (x_src)
    ...
    def text2music_diffusion_process(
        ...
        ref_latents=None,
        replace_embeds=False,
        replace_embeds_params={},
    ):
        # logger.info(
        ...
        if replace_embeds is True:
            assert "timestep_start" in replace_embeds_params, (
                "timestep_start is required"
            )
            assert 0 <= replace_embeds_params["timestep_start"] < infer_steps, (
                "timestep_start must be between 0 and infer_steps"
            )
            assert "replace_text_embeds" in replace_embeds_params, (
                "replace_text_embeds is required"
            )
            assert "replace_text_mask" in replace_embeds_params, (
                "replace_text_mask is required"
            )
            assert "replace_text_embeds_null" in replace_embeds_params, (
                "replace_text_embeds_null is required"
            )
        ...
        frame_length = math.ceil((duration * 44100) / 512 / 8)
        ...
        if latents is not None:
            ...
            assert latents.shape == (
                ...
                f"latents must have shape ({bsz}, 8, 16, {frame_length}), received {latents.shape}"
            )
        ...
        if add_retake_noise:
            ...
            if not is_repaint:
                ...
                assert target_latents.shape[-1] == x0.shape[-1], (
        ...
        if replace_embeds is True:
            encoder_hidden_states_replace, encoder_hidden_mask_replace = (
                self.ace_step_transformer.encode(
                    replace_embeds_params["replace_text_embeds"],
                    replace_embeds_params["replace_text_mask"],
                    speaker_embds,
                    lyric_token_ids,
                    lyric_mask,
                )
        ...
        if use_erg_lyric:
            ...
            if replace_embeds is True:
                encoder_hidden_states_null_replace = forward_encoder_with_temperature(
                    self,
                    inputs={
                        "encoder_text_hidden_states": replace_embeds_params[
                            "replace_text_embeds_null"
                        ]
                        if replace_embeds_params["replace_text_embeds_null"] is not None
                        else torch.zeros_like(
                            replace_embeds_params["replace_text_embeds"]
                        ),
                        "text_attention_mask": replace_embeds_params[
                            "replace_text_mask"
                        ],
                        "speaker_embeds": torch.zeros_like(speaker_embds),
                        "lyric_token_idx": lyric_token_ids,
            ...
            if replace_embeds is True:
                encoder_hidden_states_null_replace, _ = (
                    self.ace_step_transformer.encode(
                        torch.zeros_like(replace_embeds_params["replace_text_embeds"]),
                        replace_embeds_params["replace_text_mask"],
                        torch.zeros_like(speaker_embds),
                        torch.zeros_like(lyric_token_ids),
                        lyric_mask,
        ...
        encoder_hidden_states_no_lyric_replace = None
        if do_double_condition_guidance:
            ...
            if use_erg_lyric:
                ...
                if replace_embeds is True:
                    encoder_hidden_states_no_lyric_replace = (
                        forward_encoder_with_temperature(
                            self,
                            inputs={
                                "encoder_text_hidden_states": replace_embeds_params[
                                    "replace_text_embeds"
            ...
            # P(null_speaker, text, no_lyric)
                ...
                if replace_embeds is True:
                    encoder_hidden_states_no_lyric_replace, _ = (
                        self.ace_step_transformer.encode(
                            replace_embeds_params["replace_text_embeds"],
                            replace_embeds_params["replace_text_mask"],
                            torch.zeros_like(speaker_embds),
        ...
        for i, t in tqdm(enumerate(timesteps), total=num_inference_steps, disable=True):
            if is_repaint:
                if i < n_min:
                    continue
                elif i == n_min:
            ...
            enc_mask = encoder_hidden_mask
            enc_hs_no_lyric = encoder_hidden_states_no_lyric
            enc_hs_null = encoder_hidden_states_null

            if replace_embeds is True and i >= replace_embeds_params["timestep_start"]:
                enc_hs = encoder_hidden_states_replace
                enc_mask = encoder_hidden_mask_replace
                enc_hs_no_lyric = encoder_hidden_states_no_lyric_replace
                enc_hs_null = encoder_hidden_states_null_replace
            ...
            if is_in_guidance_interval and do_classifier_free_guidance:
                ...
                if do_double_condition_guidance and enc_hs_no_lyric is not None:
    ...
    def latents2audio(
        ...
        for i in tqdm(range(bs), disable=True):
    ...
    def infer_latents(self, input_audio_path):
        ...
        latents, latent_lengths = self.music_dcae.encode(input_audio, sr=sr)

        audio_duration = input_audio.shape[2] / sr
        expected_frame_length = math.ceil(audio_duration * 44100 / 512 / 8)

        actual_frame_length = latents.shape[-1]
        if actual_frame_length != expected_frame_length:
            print(
                f"infer_latents: Dimensions mismatch! Actual={actual_frame_length}, Expected={expected_frame_length}"
            )
            if actual_frame_length < expected_frame_length:
                # Pad
                latents = torch.nn.functional.pad(
                    latents, (0, expected_frame_length - actual_frame_length)
                )
            else:
                # Truncate
                latents = latents[..., :expected_frame_length]
        return latents
    ...
    def __call__(
        ...
        if ref_audio_input is not None and audio2audio_enable:
            assert ref_audio_input is not None, (
